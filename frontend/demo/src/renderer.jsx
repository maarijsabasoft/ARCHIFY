...
import { AuthProvider, LoginModal, SignupModal, VerificationModal, ForgotPasswordModal, ResetPasswordModal, UserMenu, ErrorModal } from './ui/auth-modals.jsx';
  ...
  render() {
        ...
        <style>{`
          ...
          .sidebar-panel {
            background: linear-gradient(135deg, #2d3748 0%, #4a5568 100%) !important;
              ...
              /* Ensure toolbar, sidebar, and footerbar text/icons are visible on gradient */
              [class*="toolbar"] *,
              [class*="sidebar"] *,
              [class*="footerbar"] *,
              [class*="FooterBar"] * {
    ...
    if (this.state.currentView === 'tool') {
      ...
      if (designToLoad) {
        ...
        // For new scenes, set initial zoom to fit the full grid
        setTimeout(() => {
          // Calculate zoom level to fit the entire scene (3000x2000) in viewport
          const sceneWidth = 3000;
          const sceneHeight = 2000;
          const viewportWidth = window.innerWidth - 280; // Account for sidebar
          const viewportHeight = window.innerHeight - 100; // Account for toolbar

          // Calculate zoom to fit the scene
          const zoomX = viewportWidth / sceneWidth;
          const zoomY = viewportHeight / sceneHeight;
          const fitZoom = Math.min(zoomX, zoomY) * 0.8; // 80% to add some margin

          // Set initial camera view to fit the scene
          store.dispatch({
            type: 'UPDATE_ZOOM_SCALE',
            scale: fitZoom
          });

          // Also set the camera view to center the scene
          const cameraView = {
            a: fitZoom,
            b: 0,
            c: 0,
            d: fitZoom,
            e: -sceneWidth * fitZoom / 2 + viewportWidth / 2,
            f: -sceneHeight * fitZoom / 2 + viewportHeight / 2
          };
      ...
      // Set initial zoom to fit full grid on tool load
      setTimeout(() => {
        const sceneWidth = 3000;
        const sceneHeight = 2000;
        const viewportWidth = window.innerWidth - 280; // Account for sidebar
        ...
        <TourProvider>
          <div style={{
            ...
            <style>{`
              ...
              /* Override blue theme colors - replace with site theme */
              [class*="toolbar"] button,
              [class*="toolbar"] .button,
              [class*="sidebar"] button,
              [class*="sidebar"] .button {
              ...
              /* Override React Planner footerbar background */
              .footerbar, .footer-bar, [class*="footerbar"], [class*="footer-bar"], [class*="FooterBar"] {
                background-color: rgb(40, 41, 45) !important;
                border: none !important;
                border-top: 1px solid rgba(255, 255, 255, 0.1) !important;
              }

              /* Override the specific footerbar container background - AGGRESSIVE OVERRIDE */
              [class*="footerbar"] > div:first-child,
              [class*="FooterBar"] > div:first-child,
              .footerbar > div:first-child,
              div[style*="background-color: rgb(193 222 246)"],
              div[style*="background-color: rgb(193,222,246)"],
              div[style*="background-color: rgb(193"],
              [class*="footerbar"] div,
              [class*="FooterBar"] div,
              ...
              [class*="footerbar"] [class*="background"],
              [class*="FooterBar"] [class*="background"],
              .footerbar-background,
              .footerbar-bg,
              .footerbar-wrapper,
              .footerbar-main,
              .footerbar-content {
