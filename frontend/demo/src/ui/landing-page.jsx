...
const LandingPage = ({ onGetStarted, onShowAI, onShowGuide }) => {
  ...
  var [authLoading, setAuthLoading] = React.useState(false);
  var [authSuccess, setAuthSuccess] = React.useState(false);
  var [processedAuth, setProcessedAuth] = React.useState(false);
  var [forceRerender, setForceRerender] = React.useState(0);

  // Handle OAuth authentication success
  React.useEffect(() => {
    // Only run once and check for OAuth parameters
    if (processedAuth) return;
    ...
    if (token && !auth.user) {
      ...
      // Fetch user data
      fetch('https://archify.mirdemy.com/api/auth/me', {
        headers: {
      ...
      .then(data => {
        if (data.success && data.user) {
          ...
          // Clean up URL (remove token parameters)
          ...
          console.error('Authentication failed:', data.error || 'No user data');
          setAuthLoading(false);
      ...
      // No OAuth parameters, mark as processed
      setProcessedAuth(true);
    }
  }, [auth, processedAuth]);
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', paddingBottom: '53.125%', height: 0 }}>
              <iframe
                src="https://www.loom.com/embed/31d3ad8f02984f60b9d76d31089961c2"
                frameBorder="0"
                allowFullScreen
                style={{
                  position: 'absolute',
      ...
      <footer style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{
            ...
            {/* Center - CTA section with full width */}
            <div style={{ width: '100%', maxWidth: '600px', textAlign: 'center' }}>
              <div className="fade-in-up footer-cta-section" style={{ marginBottom: '30px' }}>
                ...
                <div className="footer-cta-buttons" style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', justifyContent: 'center' }}>
                  <button
