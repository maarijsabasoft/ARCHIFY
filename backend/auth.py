...
def init_oauth(oauth_instance):
    ...
    google = oauth.register(
        ...
        client_kwargs={
            ...
            'redirect_uri': 'https://archify.mirdemy.com/api/auth/google/callback'
        }
    )
...
def verify_email():
    ...
    try:
        ...
        user.last_login_at = datetime.utcnow()  # Set login time after verification
        db.session.commit()
...
def reset_password():
    ...
    try:
        ...
        user.last_login_at = datetime.utcnow()  # Set login time after password reset
        db.session.commit()
    ...
    try:
        ...
        if user:
            ...
            user = User(
                ...
                google_id=google_id,
                email_verified=True,
                last_login_at=datetime.utcnow()  # Set initial login time
            )
            db.session.add(user)
            db.session.commit()
...
def get_me():
    ...
    # Include login status in response
    user_data = user.to_dict()
    user_data['status'] = 'signed_in' if user.last_login_at else 'not_signed_in'
...
def google_auth_callback():
    ...
    try:
        ...
        # Redirect directly to home page with token parameters
        success_url = f"{FRONTEND_URL}/?token={auth_token}&user_id={user.id}"
